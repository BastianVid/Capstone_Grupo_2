...
export function MangaView() {
  ...
  return {
    ...
    async bind() {
      ...
      let data;
      const cached = sessionStorage.getItem('mangasData');

      if (cached) {
        data = JSON.parse(cached);
      } else {
        const { ContentModel } = await import('../models/contentModel.js');
        const rawData = await ContentModel.listManga();
        ...
        sessionStorage.setItem('mangasData', JSON.stringify(data));
      }
      ...
      document.getElementById('logoutBtn')?.addEventListener('click', async () => {
        sessionStorage.removeItem('mangasData');
